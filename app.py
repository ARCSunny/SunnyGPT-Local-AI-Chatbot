...
MODEL_NAME = "gemini-3.6-flash"  # Active supported Gemini model
APP_NAME = "SunnyGPT"
APP_LOGO = "💬"
SUNNY_COLOR = "#3b82f6"
GPT_COLOR = "#10b981"
...
with st.sidebar:
  ...
  st.markdown(
      '<div class="side-label">Chat history</div>', unsafe_allow_html=True
  )
  ...
  for chat in chats:
    ...
    if pending_delete:
      ...
      with c1:
        if st.button(
            "Delete", key=f"confirm_del_{chat['id']}", use_container_width=True
        ):
          delete_chat(chat["id"])
          st.rerun()
      with c2:
        if st.button(
            "Cancel", key=f"cancel_del_{chat['id']}", use_container_width=True
        ):
          st.session_state.chat_pending_delete = None
          st.rerun()
  ...
  st.markdown(
      f"<hr style='border-color: {C['border']}; margin: 10px 0;'>",
      unsafe_allow_html=True,
  )
  theme_label = (
...
if not messages:
  st.markdown(
      f'<div class="empty-title">{APP_LOGO} <span style="color:'
      f' {SUNNY_COLOR};">Sunny</span><span'
      f' style="color: {GPT_COLOR};">GPT</span></div>',
      unsafe_allow_html=True,
  )
  st.markdown(
      '<div class="empty-sub">Ask me anything to get started.</div>',
      unsafe_allow_html=True,
  )
else:
  for msg in messages:
...
if user_input:
  ...
  store.add_message(
      st.session_state.current_chat_id, "assistant", response_text
  )
  st.rerun()